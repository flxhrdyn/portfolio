...
export default function ProfilePhoto() {
  ...
  // Idle: 0deg flat upright.
  // Hovered: Miring ke kiri (rotateZ: -12deg, rotateY: 16deg, rotateX: 4deg)
  const targetRotateX = isHovered ? 4 : 0;
  const targetRotateY = isHovered ? 16 : 0;
  const targetRotateZ = isHovered ? -12 : 0;

  const rawRotateX = useTransform(mouseY, [-0.5, 0.5], [targetRotateX + 4, targetRotateX - 4]);
  const rawRotateY = useTransform(mouseX, [-0.5, 0.5], [targetRotateY - 5, targetRotateY + 5]);
      ...
      >
        ...
        <div className="scale-portrait-sheet sheet-front-1" aria-hidden="true">
          <svg className="sheet-svg-canvas" viewBox="0 0 400 520" fill="none" stroke="currentColor">
            ...
            {/* Horizontal Dotted Projection Rays */}
