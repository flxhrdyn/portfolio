# Design Specification: Dual-Surface Hero Redesign
...
Transform both Hero surfaces from standard 50/50 template layouts into distinctive, production-grade AI engineering interfaces that communicate high-level technical credibility without visual clutter or try-hard aesthetics.
...
- **File**: `src/components/PortfolioHero.tsx` + `src/components/ProfilePhoto.tsx` (enhanced or integrated)
- **Structure**:
  - **Left Column**:
  ...
  - **Right Column (The 3D DGX Compute HUD)**:
    ...
    - **Computer Vision Brackets**: Precision corner ticks `┌ ┐ └ ┘` rendered with zero-latency SVG / pseudo-elements.
    - **Inference Laser Scan Sweep**: On mouse enter / hover, a clean, 1px horizontal light beam glides top-to-bottom across the photo once (duration 0.6s) with subtle trailing fade.
...
- **File**: `src/components/ChatHero.tsx` + `src/components/SynapticMeshCanvas.tsx` (new reusable canvas component)
- **Structure**:
  - **Background (`SynapticMeshCanvas.tsx`)**:
    ...
    - Automatically pauses when out of viewport or when `prefers-reduced-motion` is active.
  - **Foreground**:
...
## 4. Performance & Accessibility (Impeccable Standards)
1. **0px Layout Shift**: All cards, badges, and text bounds are fixed to avoid reflow.
2. **GPU-Accelerated**: 3D tilt and laser sweeps use `transform: translate3d(...)` and CSS variables (`--mouse-x`, `--mouse-y`) avoiding expensive JS re-renders.
3. **Reduced Motion**: Respects `useReducedMotion()`, gracefully degrading 3D tilt and particle animations to clean static presentation.
...
2. **Interaction Testing**: Verify 3D tilt tracking, laser sweep trigger, and synaptic mesh reactivity without frame drops.
